#Vanialla gru architecture with a weight updation after each step of prediction 
...
class GRUNet(nn.Module):
    ...
    def forward(self, observed_step,ht):
...
def train(gru_net,args):
	...
	name="estep" # to add to the name of files
	# if (cur_dataset == "eth"):
	# gru_net = VanillagruNet()
	...
	for i in range(num_epoch):
		...
		for i, batch in enumerate(dataloader):
			...
			output_seq=[]
			ht = torch.zeros(train_batch.size(1), gru_net.rnn_size, dtype=torch.float)
			ct = torch.zeros(train_batch.size(1), gru_net.rnn_size, dtype=torch.float)
			for step in range(seq):
				out,ht = gru_net(train_batch[step,:,:],ht) # forward pass of gru network for training
			# print("out's shape:", out.shape)
			for step in range(pred_len):
				out,ht=gru_net(out,ht)
				output_seq+=[out]
				optimizer.zero_grad()
...
def test(gru_net,args,pred_len):
	...
	for i, batch in enumerate(dataloader):
		...
		output_seq=[]
		ht = torch.zeros(test_observed_batch.size(1), gru_net.rnn_size, dtype=torch.float)
		ct = torch.zeros(test_observed_batch.size(1), gru_net.rnn_size, dtype=torch.float)
		for step in range(seq):
			out,ht = gru_net(test_observed_batch[step,:,:],ht) # forward pass of gru network for training
		# print("out's shape:", out.shape)
		for step in range(pred_len):
			out,ht=gru_net(out,ht)
			output_seq+=[out]
