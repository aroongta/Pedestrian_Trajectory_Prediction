...
data_dir = os.path.join('/home/ashishpc/Desktop/sgan_ab/scripts/datasets/', cur_dataset + '/train')
...
def test(gru_net,args,pred_len=0):
    ...
    test_data_dir = os.path.join('/home/ashishpc/Desktop/sgan_ab/scripts/datasets/', cur_dataset + '/train')
...
def main(args):
    ...
    if (cur_dataset == "eth"):
        ...
        if (cur_dataset == "zara1"):
            load_dataset = "eth"
        elif (cur_dataset == "zara2"):
            load_dataset = "zara1"
        elif (cur_dataset == "univ"):
            load_dataset = "zara2"
        elif (cur_dataset == "hotel"):
            load_dataset = "univ"
        print("loading dataset trained recently on {}...".format(load_dataset))
        gru_net = torch.load('./saved_models/gru_model_' + load_dataset + '_lr_' + str(args.learning_rate) + '_epoch_' + str(args.num_epochs) + '_predlen_' + str(args.pred_len) + '.pt')
    ...
    for i in range(num_epoch):
        ...
        def closure():
            for i, batch in enumerate(dataloader):
                ...
                out = gru_net(train_batch, pred_len=pred_len) # forward pass of lstm network for training
    ...
    save_path = os.path.join('./saved_models/', 'gru_model_' + cur_dataset + '_lr_' + str(learning_rate) + '_epoch_' + str(num_epoch) + '_predlen_' + str(pred_len) + '.pt')
    ...
    plt.legend()
    plt.savefig("./saved_figs/" + "gru_avgtrainloss_lr_"+ str(learning_rate) + '_epochs_' + str(num_epoch) + '_predlen_' + str(pred_len) + '.jpeg')
    # plt.show()
    # plt.show(block=True)
    ...
    plt.legend()
    # plt.show()
    plt.savefig("./saved_figs/" + "gru_avg_final_displacement_lr_"+ str(learning_rate) + '_epochs_' + str(num_epoch) + '_predlen_' + str(pred_len) + '.jpeg')

    plt.figure()
    plt.title("Std of train loss vs epoch{} epochs".format(num_epoch))
    plt.plot(std_train_loss)
    plt.savefig("./saved_figs/" + "gru_stdtrainloss_lr_"+ str(learning_rate) + '_epochs_' + str(num_epoch) + '_predlen_' + str(pred_len) + '.jpeg')
    # plt.show(block=True)
    print("saved images for avg training losses! location: " + "./saved_figs")

    # save results to text file
    txtfilename = os.path.join("./txtfiles/", "gru_avgtrainlosses_lr_"+ str(learning_rate) + '_epochs_' + str(num_epoch) + '_predlen_' + str(pred_len) + ".txt")
